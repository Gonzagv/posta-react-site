...
const Payment = function(){
    ...
    const [props3, set3] = useSpring(() => ({ xys: [0, 0, 1], config: { mass: 5, tension: 350, friction: 40 } }));
    const [props4, set4] = useSpring(() => ({ xys: [0, 0, 1], config: { mass: 5, tension: 350, friction: 40 } }));
    const [props5, set5] = useSpring(() => ({ xys: [0, 0, 1], config: { mass: 5, tension: 350, friction: 40 } }));
    return(
        <Div id="Payment">
            <Container>
                    ...
                    <Col xs="6">
                        <SpanTransferencia>
                            <animated.i class="fas fa-university fa-2x" 
                                        onMouseMove={({ clientX: x, clientY: y }) => set5({ xys: calc(x, y) })}
